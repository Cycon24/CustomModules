...
class SwirlerMeshGenerator():
    def __init__(self, **kwargs):
        ...
        # Logging Settings
        # All logging options made by GPT since I have no experience with it
        self.log_to_file       = kwargs.get('LogToFile', False)
        self.log_capture_print = kwargs.get('LogCapturePrints', True)
        self.log_dir           = Path(kwargs.get('LogDir', kwargs.get('FileLocation', '.')))
        self.log_base          = kwargs.get('LogBaseName', kwargs.get('MeshName', 'mesh'))
        self._logger           = getattr(self, '_logger', None) or logging.getLogger(f"SwirlerMeshGenerator.{id(self)}")
        

    def _make_logger(self):
        import logging
        logger = logging.getLogger(f"SwirlerMeshGenerator.{id(self)}")
        ...
        if not logger.handlers:
            ...
            # Optional file if enabled
            if getattr(self, "log_to_file", False):
                try:
                    (self.log_dir or Path(".")).mkdir(parents=True, exist_ok=True)
                except Exception:
                    pass
                ts = datetime.now().strftime("%Y%m%d_%H%M%S")
                self.log_path = (self.log_dir or Path(".")) / f"{getattr(self, 'log_base', 'mesh')}_{ts}.log"
    ...
    @contextmanager
    def _redirect_std_for_logging(self):
        if not getattr(self, "log_capture_print", True):
            yield
            return
        old_out, old_err = sys.stdout, sys.stderr
        logger = getattr(self, "_logger", None) or self._make_logger()
        sys.stdout = self._StreamToLogger(logger, logging.INFO)
        sys.stderr = self._StreamToLogger(logger, logging.ERROR)
        try:
            yield
        finally:
            try:
    ...
    def GenerateMesh(self, OpenGMSHVisual=False)-> bool:
        ...
        # --- begin optional per-call logging redirection ---
        try:
            self._logger
        except Exception:
            pass
        self._logger = getattr(self, "_logger", None) or self._make_logger()
        _log_cm = self._redirect_std_for_logging()
        _log_cm.__enter__()
        self._active_log_cm = _log_cm
        try:
            self._logger.info(f"Starting mesh generation: name={self.MeshName}")
        except Exception:
            pass
        # --- end optional per-call logging redirection ---
        ...
        # =============================================================================
            ...
            try:
                self._logger.exception("Mesh generation failed with an exception.")
            except Exception:
                print("Error Occured")
                print(e)
                traceback.print_exc()
            ...
            try:
                if getattr(self, "_active_log_cm", None):
                    self._active_log_cm.__exit__(None, None, None)
                    self._active_log_cm = None
                if getattr(self, "log_path", None):
                    try:
                        self._logger.info(f"Log saved to: {self.log_path}")
                    except Exception:
                        pass
            finally:
